"""
build_ieee_docx.py
Convert the pandoc base docx (_ieee_base.docx) into a true IEEE two-column
...
SRC = '_ieee_base.docx'
DST = 'Final_Project_Paper_Bakir_Baskal_IEEE.docx'
...
for p in list(d.paragraphs[:8]):
    if p.text.strip().startswith('Bak') and 'Predicting Quarterly Airbnb Booking Demand in NYC' in p.text:
        delete_para(p)
...
if inventory_heading is not None and inventory_heading > 0:
    ...
    # the final body section (already the last sectPr in the doc) now covers only
    # the inventory heading + table onward: make it single-column, full width
    for c in body_sp.findall(qn('w:cols')):
        body_sp.remove(c)
    docGrid = body_sp.find(qn('w:docGrid'))
    cols1 = make_cols(1)
    ...
    # widen the table itself to fill the ~7.25in single-column width
