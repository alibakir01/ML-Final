"""
model_run.py --data <parquet> --target <col> --tag <name>
Generic pipeline (median impute + one-hot low-card + K-fold target-encode high-card,
5-fold CV, clip [0,92]) for Linear/RF/XGBoost/LightGBM/CatBoost + SLSQP blend.
Saves OOF predictions (outputs/oof_<tag>.npz) and a results txt.
"""
import argparse
...
for n, (mk, sc) in models.items():
    ...
    print(f'{n:<18} MSE={rows[-1][1]:7.2f} MAE={rows[-1][2]:6.2f} R2={rows[-1][3]:.3f}', flush=True)
...
rep=[f'RESULTS [{a.tag}] target={a.target} (5-fold CV, n={len(y)})', '='*60, f'{"Model":<18}{"MSE":>9}{"MAE":>8}{"R2":>8}']
rep += [f'{n:<18}{mse:>9.2f}{mae:>8.2f}{r2:>8.3f}' for n,mse,mae,r2 in rows]
