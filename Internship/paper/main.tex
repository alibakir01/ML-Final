...
Section~\ref{sec:eda}; critically, none of those 2026 snapshots
...
missing \texttt{host\_response\_rate} typically marks an inactive host), we
encode missingness explicitly with indicator columns rather than discarding it.
...
the weights still sum to one exactly. We also ran a randomized search directly
...
the target-encoding formula above. As a last step we clip predictions to $[0,90]$, both
...
City); the learned price and seasonality effects need not transfer to other
cities, and the heavy reliance on calendar-volatility features means the
model would degrade for brand-new listings with no multi-month history.
Temporal generalization within this market was checked directly
(Section~\ref{sec:robustness}): we re-ran the same pipeline, with no
per-setup re-tuning, on two additional history/target configurations (Q4
2025 and Feb--Apr 2026), and held-out $R^2$ stayed in the same 0.54--0.62
range as the primary Q1 2026 setup, so this result is not a one-quarter
artefact. \emph{Construct validity:} the
...
genuinely booked (Section~\ref{sec:eda}), which was not a
...
Everything reported so far uses a single history/target configuration --- we
call it Setup B --- July--December 2025 history predicting Q1 2026. To check
that this is not a one-quarter result, we re-ran the identical pipeline (the
clean multi-snapshot target rule, the same five-model roster with
hyperparameters reused unchanged from Setup B, and the same SLSQP blend) on
two additional configurations: Setup A (July--September 2025 history
predicting Q4 2025, the shallowest-history case) and Setup C (July
2025--January 2026 history predicting February--April 2026, the
deepest-history case). Table~\ref{tab:setups} summarises the three
configurations, and Table~\ref{tab:targetstats3} their target distributions,
each built from the identical clean multi-snapshot rule
...
Figure~\ref{fig:robustness} reports the held-out test $R^2$ of the SLSQP blend
for all three setups; Figure~\ref{fig:robustnessmodels} breaks this down by
individual model. The blend's held-out $R^2$ is 0.622 (Setup A), 0.536 (Setup
B), and 0.567 (Setup C): the accuracy is not a one-quarter artefact, and the
same model ranking (blend $>$ CatBoost $\approx$ LightGBM $\approx$ XGBoost
$>$ Random Forest $\gg$ Linear Regression) holds in every period.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig8_cross_period_robustness.png}
\caption{Cross-period robustness: held-out test $R^2$ of the SLSQP blend for
...
We also re-tested two further findings from Setup B on Setups A and C, with
all hyperparameters reused unchanged (no per-setup re-tuning). First, the
hurdle model (Section~\ref{sec:hurdle-impl}) did not improve on a single
XGBoost model in either new setup (MSE $+4.15$ for Setup A, $+3.56$ for Setup
C), matching Setup B's own borderline result. Second, we ran a direct
naive-vs-clean-target comparison (fitting the identical model on both target
definitions) for the first time on this project: it does \emph{not} show the
clean target achieving a better $R^2$ or MSE than the naive target on either
new setup --- the naive target's $R^2$ is mechanically inflated by its own
higher variance ($R^2 = 1-\mathrm{MSE}/\mathrm{Var}(y)$; the naive target has
$1.9$--$2.3\times$ the clean target's variance). We do not read this as
evidence against the clean-target design: our justification for it was always
a construct-validity argument (the clean rule isolates a real
available$\to$booked transition, whereas the naive count conflates that with
host-blocking, Section~\ref{sec:eda}), not a claim that it would score better
by these metrics, and this result is a useful reminder to state that argument
precisely rather than oversell it as an error-reduction technique.
...
spirit as the additive smoothing formula already used for target encoding. Third, with a
larger compute budget we would tune the MLP more carefully; the
error-correlation analysis earlier (Section~\ref{sec:complementarity}) showed it is the least
correlated model with the tree ensembles, so a stronger MLP is a promising way
to raise the weight the blend is willing to give it. Finally, the cross-period
check (Section~\ref{sec:robustness}) used quarters already present in the ten
available snapshots; unlike the original fixed-snapshot study, this pipeline
can also be re-run against later Inside Airbnb scrapes as they are published,
so validating it against a genuinely new quarter (e.g.\ predicting Q3 2026
once that window closes) would be a stronger, non-backtested test of the same
external-validity concerns raised in Section~\ref{sec:threats}.
