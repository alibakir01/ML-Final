...
fields---feeds five learners (Linear Regression, Random Forest, XGBoost, LightGBM,
CatBoost) that are combined by a convex weighted blend whose weights are found by
...
a $20\%$ held-out test $R^2$ of $0.653$, within $0.010$ of its cross-validation
...
$K$-fold target encoding, five heterogeneous learners, and an SLSQP convex
blend---and evaluates it with both cross-validation and a disjoint held-out test
set, reaching a held-out $R^2$ of $0.653$.
...
Five heterogeneous learners span the linear, bagging, and boosting families:
Linear Regression (a scaled interpretable baseline), Random Forest \cite{breiman2001},
XGBoost \cite{chen2016xgboost}, LightGBM \cite{ke2017lightgbm}, and CatBoost
\cite{prokhorenkova2018catboost}. All are trained on the same preprocessed matrix
...
Rather than selecting a single learner, the out-of-fold predictions of the $m=5$
...
$m=5$ weights over a cached matrix of out-of-fold predictions and converges in well
...
random-forest models, cross-validation), XGBoost, LightGBM, CatBoost, and
...
for the boosted learners, and \texttt{scipy.optimize} for the SLSQP blend; all
experiments run on a single multi-core CPU with no GPU. The code is organised as a
...
configuration on both partitions, reaching a held-out test $R^2$ of $0.653$
(MSE $262.0$). Figure~\ref{fig:modelcmp} visualises the held-out ranking.
Critically, the held-out $R^2$ ($0.653$) is within $0.010$ of the development
cross-validation estimate ($0.663$): the model does not overfit, and the
...
\textbf{SLSQP blend} & \textbf{0.663} & \textbf{0.653} & \textbf{262.0} \\
\bottomrule
\end{tabular}
\end{table}
...
weights concentrate on the complementary learners---Random Forest $0.39$, LightGBM
$0.36$, XGBoost $0.23$, CatBoost $0.02$, Linear $0.00$---rather than placing all
...
\textbf{SLSQP blend} & \textbf{257.0} & \textbf{9.66} & \textbf{0.664} \\
\bottomrule
\end{tabular}
\end{table}
...
A & Q4 2025 & 0.645 \\
B & Q1 2026 & 0.574 \\
\textbf{C} & \textbf{Feb--Apr 2026} & \textbf{0.664} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig7_residuals.png}
...
before the target quarter), the platform's own trailing occupancy estimate, the
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig9_importance.png}
\caption{XGBoost gain importances (Setup~C). Recent behavioural features---the
latest month's booking rate and count, trailing occupancy, and review
recency---dominate; static metadata is secondary.}
\label{fig:imp}
\end{figure}
...
On the held-out test the two-stage hurdle of \eqref{eq:hurdle} attains an
\emph{identical} $R^2$ ($0.653$) to the direct blend; its small cross-validation
advantage ($+0.002$) does not survive on unbiased data. The gradient-boosted blend
...
blend of five heterogeneous learners, trained on a seven-month behavioural history,
attained a $20\%$ held-out test $R^2$ of $0.653$ that closely matched its
