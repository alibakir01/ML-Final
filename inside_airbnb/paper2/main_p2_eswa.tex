...
% Standalone, double-blind Elsevier (ESWA) single-column submission manuscript.
% Build: upload the paper2/ folder to Overleaf and compile main_p2_eswa.tex
% (companion double-column IEEEtran version: main_p2.tex)
%==============================================================================
...
\title{An Intelligent Predictive Framework for Short-Term Rental Demand:
Snapshot-Differencing and Convex Ensembling on Zero-Inflated Data}

\author{Anonymous Author(s)\corref{cor1}}
\cortext[cor1]{Author names and affiliations omitted for double-blind review.}

\begin{abstract}
Accurately forecasting how heavily a short-term rental will be booked is valuable
for hosts, platforms, and regulators, yet most public data expose only coarse
calendar availability rather than explicit bookings. This paper studies quarterly
demand forecasting for New York City short-term rentals using ten monthly Inside
Airbnb snapshots (July 2025--April 2026). Because the calendars provide only an
\texttt{available} (true/false) flag and no usable price, the target (booked
nights in a future quarter) is reconstructed by \emph{snapshot differencing}: a
date that transitions from available to unavailable and stays unavailable is
counted as booked. The resulting target is a censored, zero-inflated integer in
$[0,92]$ with half its mass at zero. A single leakage-safe pipeline (median
imputation, one-hot encoding, and $K$-fold target encoding for high-cardinality
fields) feeds six learners (Linear Regression, Random Forest, XGBoost, LightGBM,
CatBoost, and a PyTorch multi-layer perceptron), combined by a convex weighted
blend with weights from Sequential Least-Squares Quadratic Programming. Using a
seven-month behavioural history to predict the February--April 2026 quarter, the
blend attains a held-out test $R^2$ of $0.654$, within $0.011$ of its
cross-validation estimates, and its improvement over the best single learner is
statistically significant. The method is validated on three independent
quarters, and an ablation shows that the dominant lever is the target definition
and the depth of behavioural history; hyper-parameter tuning, a two-stage hurdle
model, enriched price features, and review-velocity features add little.
Achievable accuracy on this public source is bounded primarily by label quality,
not model capacity.
\end{abstract}

\begin{keyword}
Short-term rental demand forecasting \sep Inside Airbnb \sep zero-inflated regression \sep gradient boosting \sep target encoding \sep convex model blending \sep snapshot differencing
\end{keyword}
...
Short-term rental (STR) platforms such as Airbnb have reshaped
...
itself. Much of the earlier STR-modelling literature relied on richer public
datasets that exposed an explicit per-day booking or reservation record and a
per-day price. Contemporary public snapshots---most prominently those distributed
by the Inside Airbnb project \citep{insideairbnb}---have converged on a much coarser
...
for price prediction. Tang, Cheng, and Zhang \citeyearpar{tang2024} compared ten
algorithms on Sydney listings and found gradient-boosted trees, and CatBoost in
particular, to be strongest. Camatti et al.\ \citeyearpar{camatti2024} contrasted
...
Rezazadeh Kalehbasti et al.\ \citeyearpar{kalehbasti2019} predicted New York City prices
from property attributes, host characteristics, and review sentiment. Islam et
al.\ \citeyearpar{islam2022} combined a topic model over descriptions with an XGBoost
...
The decision tree \citep{quinlan1986} yields interpretable, non-parametric
regressors, but a single tree overfits. Bagging \citep{breiman1996} and Random
Forests \citep{breiman2001} reduce variance by averaging decorrelated trees, while
gradient boosting \citep{friedman2001} reduces bias by fitting residuals
sequentially. XGBoost \citep{chen2016xgboost}, LightGBM \citep{ke2017lightgbm}, and
CatBoost \citep{prokhorenkova2018catboost} are widely used, highly optimised
...
final error \citep{pedregosa2011sklearn}. High-cardinality categoricals such as
...
accuracy. Stacked generalisation \citep{wolpert1992stacked} learns a meta-model
...
positive count, and censored-regression estimators such as Tobit \citep{tobin1958}
treat the bound explicitly. Dietterich \citeyearpar{dietterich1998} recommends paired
...
The study uses public Inside Airbnb snapshots \citep{insideairbnb} for New York
...
Fig.~\ref{fig:avail} shows, roughly $56\%$ of calendar-days are unavailable,
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig1_availability.png}
\caption{Calendar availability. About $56\%$ of calendar-days are unavailable
(September 2025 snapshot), reflecting heavy host blocking and dormancy; the raw
``unavailable'' count is therefore a poor proxy for realised demand.}
\label{fig:avail}
\end{figure}

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig2_price_coverage.png}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig3_review.png}
...
mean $19.2$, median $2$, standard deviation $27.7$, and $47.7\%$ of listings at
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig4_target_dist.png}
...
\emph{left}-skewed with a median of $66$ of $92$ nights and just over $10\%$
...
\begin{table}[!t]
\caption{Naive occupancy counts vs.\ the differencing target (Setup~C,
Feb--Apr 2026; same listings and quarter). Only the differencing target is
zero-inflated and demand-like.}
\label{tab:targetdef}
\centering
\small
...
\midrule
Occupancy (unavailable days) & 54.0 & 66 & 36.2 & 10.7 & $-0.38$ \\
\textbf{Differencing (booked)} & 19.2 & 2 & 27.7 & 47.7 & $+1.28$ \\
\bottomrule
\end{tabular}
\end{table}
...
keyword flags are extracted from the listing title and description. Crucially, forward-looking
...
high-cardinality categorical fields (neighbourhood and property type) are handled
by a $K$-fold target encoder with additive smoothing. Writing $\mu$ for the global target mean and,
...
\citep{breiman2001}, XGBoost \citep{chen2016xgboost}, LightGBM \citep{ke2017lightgbm},
CatBoost \citep{prokhorenkova2018catboost}, and a PyTorch \citep{paszke2019pytorch}
multi-layer perceptron trained with the Adam optimiser \citep{kingma2015adam}. The
...
$\log(1+y)$ transform, effective in an earlier formulation of this problem on
historical data, degraded its accuracy here. Adam updates each parameter using the standard moment
...
leaf count $T_k$ and leaf-weight norm, discouraging overly complex trees;
$\lambda=3$ is a manually set default value for this parameter, close to but not
identical with the value ($\approx2.0$) found by the automated search of
Section~\ref{sec:hpsearch}, which did not meaningfully improve on this default.
...
retained for comparison. The reported hurdle figure in Section~\ref{sec:ablation}
is itself a three-way convex blend of the XGBoost- and LightGBM-based hurdle
experts with the plain six-learner blend (held-out weights $\approx0.19/0.21/0.60$);
a pure, single hurdle expert alone scores lower, so the comparison should be read
as blend vs.\ blend rather than hurdle-alone vs.\ blend. The empirical
outcome---that neither variant improves on the direct blend---is reported and
discussed in Section~\ref{sec:ablation}.

\subsection{Hyper-parameter search}
\label{sec:hpsearch}
The two boosted-tree learners most likely to benefit from tuning, XGBoost and
LightGBM, were tuned by a randomised search of $30$ configurations per family
...
\label{sec:compcost}
The pipeline is inexpensive on commodity hardware. Wall-clock timings were
obtained by re-running the full $5$-fold cross-validation pipeline for all three
setups (A, B, C) on a MacBook Air (Apple M4, 10-core CPU, 16GB RAM, 512GB SSD);
the resulting $R^2$ values matched the recorded results exactly, confirming that
the reported figures are reproducible on independent hardware. Table~\ref{tab:timing}
reports the per-model fit time for each setup. Linear Regression is fastest
($1.5$--$2.1$s); XGBoost, using \texttt{tree\_method='hist'}, trains in under $15$
seconds despite being one of the strongest single learners---faster even than
LightGBM and CatBoost. Random Forest, with $300$ fully-grown trees, is the single
most expensive model ($158$--$208$s per setup)---roughly $70\%$ of the combined
tree/linear-model time, or $53$--$57\%$ of total wall-clock time once the MLP is
included. The SLSQP blend converges in well under a second regardless of setup,
confirming that ensembling adds negligible overhead once the six base learners are
trained. Total end-to-end runtime, including the MLP now trained for all three
setups, is about $5$ minutes for Setup~A ($68$ features, $\sim$5:01) and Setup~C
($75$ features, $\sim$4:59), and about $6$ minutes for Setup~B ($99$ features, the
largest feature set, $\sim$6:04); building the feature matrix from the
multi-million-row calendars is a one-time aggregation that dominates overall
wall-clock time but is trivially parallel over listings. Because the out-of-fold
predictions are cached, the blend and every post-hoc diagnostic can be recomputed
without retraining, which makes the entire ablation study cheap to run.

\begin{table}[!t]
\caption{Wall-clock training time per model (5-fold CV) for the three setups of
Table~\ref{tab:setups}, measured on a MacBook Air (Apple M4, 10-core CPU, 16GB RAM,
512GB SSD). Setup~A: $n=36{,}111$, 68 features; Setup~B: $n=36{,}282$, 99 features;
Setup~C: $n=36{,}282$, 75 features.}
\label{tab:timing}
\centering
\small
\begin{tabular}{lccc}
...
Model & A & B & C \\
\midrule
Linear Regression & 1.7s & 2.1s & 1.5s \\
Random Forest & 161.2s & 208.3s & 157.5s \\
XGBoost & 13.3s & 14.7s & 11.3s \\
LightGBM & 38.4s & 41.5s & 38.0s \\
CatBoost & 18.3s & 22.8s & 18.4s \\
MLP & 68.5s & 74.1s & 72.6s \\
SLSQP blend & $<$0.1s & $<$0.1s & $<$0.1s \\
\midrule
\textbf{Total (script)} & \textbf{$\sim$5:01} & \textbf{$\sim$6:04} & \textbf{$\sim$4:59} \\
\bottomrule
\end{tabular}
\end{table}
...
cross-validation partitions, and the inner target-encoding folds. Out-of-fold
predictions are cached so that the blend and all diagnostics can be recomputed
without retraining. The complete software stack and execution hardware are
consolidated in Table~\ref{tab:stack}; all experiments were run on a MacBook Air
(Apple M4, 10-core CPU, 16GB RAM, 512GB SSD) with no discrete GPU. Execution times
are reported in Section~\ref{sec:compcost}. The code is organised as a
...
\begin{table}[!t]
\caption{Unified software stack and execution environment used for all
experiments.}
\label{tab:stack}
\centering
\small
...
\midrule
Python (language / runtime) & 3.13 \\
scikit-learn (preprocessing, linear and random-forest models, CV) & 1.8.0 \\
XGBoost & 3.2.0 \\
LightGBM & 4.6.0 \\
PyTorch (MLP sixth learner) & 2.12.0 \\
CatBoost & 1.2.10 \\
SciPy (\texttt{scipy.optimize}, SLSQP blend) & 1.17.0 \\
Execution hardware & MacBook Air, Apple M4 (10-core CPU), 16GB RAM, 512GB SSD, no discrete GPU \\
Random seed & Single fixed seed governing the development/test split, CV partitions, and target-encoding folds \\
Wall-clock times & Per-model 5-fold CV fit times in Table~\ref{tab:timing} \\
\bottomrule
\end{tabular}
\end{table}
...
A note on external comparability is warranted. Because the target booked-nights
label is reconstructed via our custom snapshot-differencing methodology on a
unique temporal window (2025--2026), direct numerical comparisons against prior
historic baselines are mathematically non-viable. Instead, the method is validated
internally through a comparison across six structurally diverse learner families
(linear, tree-ensemble, and neural) trained under identical, leakage-free
conditions on the same data, with the convex blend consistently outperforming
every individual family (Tables~\ref{tab:heldout} and~\ref{tab:devcv}).
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig5_model_compare.png}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig6_temporal.png}
...
On a paired test of per-listing squared errors, Setup~A gives $t=7.11$,
$p=1.16\times10^{-12}$ and Setup~B gives $t=7.20$, $p=6.31\times10^{-13}$; in both
...
$2{,}068$ listings with $y\ge 80$, the blend predicts a mean of only $61.3$ nights,
...
3401271 & 89 & 3.3 & 0.00 \\
39598996 & 89 & 3.4 & 0.00 \\
649267754730612723 & 89 & 3.9 & 0.00 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Differencing-rule refinement ($y_{\text{orig}}\rightarrow y_{\text{clean}}$, & $+0.04$ & small, reproducible \\
\quad $\ge$2-obs.\ filter) & & refinement within \\
 & & the differencing family \\
Naive occupancy vs.\ differencing & $-0.15$ & $R^2$ not comparable \\
\quad target (Setup~C) & & see discussion \\
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig10_ablation.png}
...
\paragraph{Target definition: why raw $R^2$ cannot justify the choice}
A naive comparison of $R^2$ would appear to favour the wrong target. Fitting the
identical six-learner pipeline to the raw occupancy count (unavailable
calendar-days in the target quarter) rather than the differencing target defined
in Section~\ref{sec:data} yields a substantially \emph{higher} blend
$R^2$---$0.812$ versus $0.665$ on Setup~C (5-fold CV)---a gap of $-0.147$ in the
direction opposite to what a naive reading of ``target quality'' would predict.
Table~\ref{tab:targetdef} explains why: the occupancy count is left-skewed with
only $10.7\%$ zeros and a median of $66$ of $92$ nights, because a host's
calendar-blocking habits are highly persistent from one period to the next: a host
who blocks half their calendar in the history window is very likely to still be
blocking it in the target quarter, so the model only has to learn a near-static,
autocorrelated pattern. The differencing target, by design, strips away exactly
this persistent-blocking signal and leaves only genuine booking transitions, which
are markedly more volatile and harder to predict; its high zero-inflation
($47.7\%$ versus $10.7\%$) and positive skew ($+1.28$ versus $-0.38$,
Table~\ref{tab:targetdef}) reflect a fundamentally different and harder estimand.
$R^2$ therefore cannot be used, on its own, to compare the two target definitions:
a higher $R^2$ on the occupancy count is evidence that the model is accurately
predicting \emph{host behaviour}, not evidence that occupancy is a better demand
label. The differencing target is preferred not because it raises $R^2$, but
because it is the quantity the paper's applications actually require---genuine
guest demand rather than calendar-management habit---and
Table~\ref{tab:targetdef}'s distributional contrast is the correct basis for that
choice, not the $R^2$ gap.

A smaller, genuinely apples-to-apples comparison remains available within the
differencing family itself: tightening the differencing rule from a
single-transition read ($y_{\text{orig}}$: available-at-first-observation
$\rightarrow$ unavailable-at-last, no minimum-observation requirement) to the full
two-observation rule of $y_{\text{clean}}$ (\eqref{eq:booked}) raises the blend
$R^2$ by about $0.04$ and does so reproducibly across periods (for Setup~A, $R^2$
rises from $0.605$ to $0.645$). Because both targets are differencing-based, this
comparison is not confounded by the persistence effect above, and the gain is a
genuine, if modest, label-quality improvement: the two-observation filter removes
host-blocked dates that were never available in the first place, reducing a
specific, identifiable source of label noise.
...
\label{sec:appendixA}
Table~\ref{tab:allfeat} lists the complete inventory of all 77 columns in the
processed Setup~C feature matrix (the listing identifier, the target, and 75
engineered features), giving the exact column name and inferred data type of
every field that reaches the learners. Behavioural, property/host, review,
geographic, and text-keyword families are summarised at the family level in
Table~\ref{tab:featfam} and Table~\ref{tab:featinv}; this appendix gives the
per-column detail underlying those summaries.

\footnotesize
\begin{longtable}{lc|lc}
\caption{Detailed list of all 77 columns in the processed Setup~C feature matrix (identifier, target, and 75 features).}
\label{tab:allfeat}\\
\toprule
...
\bottomrule
\endlastfoot
\texttt{listing\_id} & integer (ID) & \texttt{instant\_bookable} & binary \\
\texttt{hq3\_booked} & float & \texttt{number\_of\_reviews} & integer \\
\texttt{hq3\_occupied} & float & \texttt{number\_of\_reviews\_ltm} & integer \\
\texttt{hq3\_obs\_days} & float & \texttt{number\_of\_reviews\_l30d} & integer \\
\texttt{hq3\_changes} & float & \texttt{reviews\_per\_month} & float \\
\texttt{hq3\_booking\_rate} & float & \texttt{review\_scores\_rating} & float \\
\texttt{hq3\_occ\_rate} & float & \texttt{review\_scores\_accuracy} & float \\
\texttt{hq4\_booked} & float & \texttt{review\_scores\_cleanliness} & float \\
\texttt{hq4\_occupied} & float & \texttt{review\_scores\_location} & float \\
\texttt{hq4\_obs\_days} & float & \texttt{review\_scores\_value} & float \\
\texttt{hq4\_changes} & float & \texttt{estimated\_occupancy\_l365d} & integer \\
\texttt{hq4\_booking\_rate} & float & \texttt{estimated\_revenue\_l365d} & float \\
\texttt{hq4\_occ\_rate} & float & \texttt{license\_missing} & binary \\
\texttt{hjan\_booked} & integer & \texttt{has\_reviews} & binary \\
\texttt{hjan\_occ} & integer & \texttt{host\_age\_days} & float \\
\texttt{hjan\_obs} & integer & \texttt{days\_since\_first\_review} & float \\
\texttt{hjan\_booking\_rate} & float & \texttt{days\_since\_last\_review} & float \\
\texttt{h7\_booked\_total} & float & \texttt{bed\_bath\_ratio} & float \\
\texttt{trend\_q4\_q3} & float & \texttt{kw\_luxury} & binary \\
\texttt{trend\_jan\_q4} & float & \texttt{kw\_private} & binary \\
\texttt{momentum} & float & \texttt{kw\_cozy} & binary \\
\texttt{recency\_booking\_rate} & float & \texttt{kw\_modern} & binary \\
\texttt{neighbourhood\_cleansed} & string & \texttt{kw\_spacious} & binary \\
\texttt{neighbourhood\_group\_cleansed} & string & \texttt{kw\_view} & binary \\
\texttt{latitude} & float & \texttt{kw\_near} & binary \\
\texttt{longitude} & float & \texttt{kw\_studio} & binary \\
\texttt{property\_type} & string & \texttt{title\_len} & integer \\
\texttt{room\_type} & string & \texttt{title\_words} & integer \\
\texttt{accommodates} & integer & \texttt{review\_scores\_rating\_missing} & binary \\
\texttt{bathrooms} & float & \texttt{host\_response\_rate\_missing} & binary \\
\texttt{bedrooms} & float & \texttt{reviews\_per\_month\_missing} & binary \\
\texttt{beds} & float & \texttt{estimated\_occupancy\_l365d\_missing} & binary \\
\texttt{minimum\_nights} & float & \texttt{geo\_cluster} & string \\
\texttt{maximum\_nights} & float & \texttt{rev\_total} & float \\
\texttt{host\_is\_superhost} & binary & \texttt{rev\_90} & float \\
\texttt{host\_response\_rate} & float & \texttt{rev\_365} & float \\
\texttt{host\_acceptance\_rate} & float & \texttt{rev\_recency\_days} & float \\
\texttt{host\_listings\_count} & float & \texttt{y\_clean} & integer (target) \\
\texttt{host\_total\_listings\_count} & float &  &  \\
\end{longtable}
\normalsize
